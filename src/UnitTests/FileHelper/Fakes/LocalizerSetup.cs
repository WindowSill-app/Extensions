...
/// Loads the <c>WindowSill.FileHelper</c> extension's <c>.resw</c> resource files so that localized string
/// lookups return real values instead of empty strings.
/// </summary>
/// <remarks>
/// Mirrors <c>UnitTests.Date.Core.Fakes.LocalizerSetup</c>, adapted to load <c>WindowSill.FileHelper</c>'s own
/// resource files (rather than <c>WindowSill.Date</c>'s) and duplicated here — instead of shared — so that
...
internal static class LocalizerSetup
{
    ...
    /// <summary>
    /// Loads all <c>.resw</c> files from the WindowSill.FileHelper extension's Strings/en-US
    /// directory and adds each entry to the <see cref="LanguageDictionary"/>.
    /// The resource UID format is <c>/WindowSill.FileHelper/{Category}/{Key}</c>, matching
    ...
    private static void LoadReswFiles(object langDict, Type langDictType, Type itemType)
    {
        ...
        foreach (string reswPath in Directory.GetFiles(stringsDir, "*.resw"))
        {
            ...
            foreach (XElement dataElement in doc.Descendants("data"))
            {
                ...
                if (dotIndex >= 0)
                {
                    uid = $"/WindowSill.FileHelper/{category}/{name[..dotIndex]}";
                    depPropName = name[(dotIndex + 1)..];
                }
                else
                {
                    uid = $"/WindowSill.FileHelper/{category}/{name}";
    ...
    private static string? FindStringsDirectory()
    {
        ...
        {
            string candidate = Path.Combine(dir, "WindowSill.FileHelper", "Strings", "en-US");
            ...
            candidate = Path.Combine(dir, "src", "WindowSill.FileHelper", "Strings", "en-US");
            if (Directory.Exists(candidate))
            {
                return candidate;
            }
